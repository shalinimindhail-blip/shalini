const config = {
  recipientName: "Handsome",
  
  pages: {
    welcome: {
      mainText: "Hi, Handsome ❤️",
      subText: "A little something just for you.",
      buttonText: "Begin"
    },
    
    gift: {
      text: "Will you accept my gift?",
      yesButton: "❤️ Yes",
      noButton: "🙈 No"
    },
    
    loading: {
      messages: [
        "Preparing your surprise...",
        "Packing lots of love...",
        "Collecting our memories...",
        "Almost ready...",
        "100% Complete ❤️"
      ]
    },
    
    memories: {
      title: "Our Beautiful Memories 📸",
      frontText: "Tap to read what I wrote on the back.",
      polaroids: [
        {
          image: "assets/images/memory1.jpg",
          message: "Remember this day? You looked so handsome, and I fell in love all over again."
        },
        {
          image: "assets/images/memory2.jpg",
          message: "This was the moment I knew you were the one for me."
        },
        {
          image: "assets/images/memory3.jpg",
          message: "Every adventure with you is an adventure worth treasuring."
        },
        {
          image: "assets/images/memory4.jpg",
          message: "Your smile brightens even my darkest days."
        },
        {
          image: "assets/images/memory5.jpg",
          message: "These moments with you are my favorite memories."
        },
        {
          image: "assets/images/memory6.jpg",
          message: "Forever grateful for you, my love."
        }
      ]
    },
    
    reasons: {
      title: "10 Reasons Why I Love You ❤️",
      reasons: [
        "Your incredible sense of humor makes me laugh every single day.",
        "The way you listen with your whole heart means everything to me.",
        "You're so thoughtful and kind to everyone around you.",
        "Your strength and determination inspire me to be better.",
        "You make me feel beautiful just by looking at me.",
        "The way you love me is pure and true.",
        "You're my biggest cheerleader and my safe place.",
        "Your passion for life is absolutely contagious.",
        "You make ordinary moments feel magical.",
        "Because you are simply the best part of my life."
      ]
    },
    
    song: {
      title: "Our Song 🎵",
      subtitle: "This one always brings me right back to you.",
      musicLink: "https://www.youtube.com/embed/dQw4w9WgXcQ",
      placeholder: "Paste your YouTube or Spotify embed link here"
    },
    
    hearts: {
      title: "Find All 5 Hidden Hearts 💕",
      secretMessage: "You found them all! 💕 I love you more than words could ever express."
    },
    
    letter: {
      title: "A Letter Just For You 💌",
      content: "My Dearest Love,\n\nI wanted to take a moment to tell you how much you mean to me. Every day with you is a gift, and I'm so grateful for your presence in my life.\n\nYou have this amazing way of making even the simplest moments feel special. The way you look at me, the way you hold my hand, the way you make me laugh – these are the things I treasure most.\n\nThank you for being patient, kind, and incredibly loving. Thank you for standing by my side through everything. You are my greatest adventure, my safest haven, and my forever love.\n\nI can't wait to spend more birthdays with you, creating countless more beautiful memories together.\n\nForever yours,\nYour Love ❤️"
    },
    
    finale: {
      title: "One Last Surprise... 🎁",
      tapText: "Tap the gift",
      message: "Happy Birthday to the love of my life. ❤️\n\nThank you for being the best part of my life.\n\nI hope this year brings you everything you've wished for, fills your life with happiness, success, peace, and countless beautiful memories.\n\nI can't wait to create many more memories together.\n\nLove you always. 💕"
    }
  },
  
  colors: {
    primary: "#ff69b4",
    secondary: "#ffd1e8",
    accent: "#ff1493",
    dark: "#2a2a2a",
    light: "#ffffff"
  }
};